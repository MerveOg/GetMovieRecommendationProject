...
public class FileOperations<T extends Comparable> {
    ...
    private static BufferedReader getBufferedReader(String fileName) {
        return new BufferedReader(new InputStreamReader(Objects.requireNonNull(FileOperations.class.getResourceAsStream(fileName))));
    }
    ...
    public static int columnNumber(String csvPath) {
        try (BufferedReader br = getBufferedReader(csvPath)) {
    ...
    public Integer[][] targetUserMatrix() {
        String targetFile = "/target_user.csv";
    ...
    private static <T extends Number> T[][] createTypedArray2D(int rows, int cols, Class<T> tClass) {
        @SuppressWarnings("unchecked")
        T[][] array = (T[][]) Array.newInstance(tClass, rows, cols);
        return array;
    }
